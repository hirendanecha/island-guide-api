...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in IslandGuideTube channel by the IslandGuide Admin.
             To access your channel, log into your IslandGuide account,click on the
             IslandGuide icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `IslandGuide has approved your guide account.`;
    } else {
      msg = `IslandGuide has approved your guide account.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
...
exports.channelCreationMail = async (adminMail, userName) => {
  let redirectUrl = "https://admin.islandguide.tours/community";
  let msg = `${userName} has create guide on IslandGuide.Tours.`;
  const mailObj = {
    email: adminMail,
    subject: "New guide registered",
    root: "../email-templates/notification.ejs",
